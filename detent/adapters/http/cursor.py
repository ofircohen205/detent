...
class CursorAdapter(OpenAICompatibleAdapter):
    ...
    async def intercept(self, raw_event: dict[str, Any]) -> AgentAction | None:
        """Normalize a Cursor tool call to AgentAction with logging.
    ...
    async def intercept_response(self, resp_body: bytes) -> list[AgentAction]:
        """Parse a Cursor response body for tool calls with logging.
