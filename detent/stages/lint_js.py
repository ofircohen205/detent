...
"""ESLint runner used by LintStage for JS/TS files."""
...
async def run_eslint(file_path: str, content: str, timeout: int) -> list[Finding]:
    ...
    try:
        ...
        logger.warning("[lint_js] eslint output was not valid JSON: %s", stdout_text[:200])
        return []

    findings: list[Finding] = []
    for result in parsed:
        findings.extend(_parse_result(file_path, result))
    return findings


def _parse_result(file_path: str, result: dict[str, list[dict[str, Any]]]) -> list[Finding]:
    findings: list[Finding] = []
